...
public final class NodeListRule implements Rule<CompoundNode, RuleResult<CompoundNode>, RuleResult<String>> {
    ...
    public RuleResult<CompoundNode> lex(String input) {
        return new FoldingDivider().divide(input).stream().reduce(RuleResult.createFromValue(new ArrayList<>()), NodeListRule.this::fold, (_, next) -> next).mapValue(children -> new PropertiesCompoundNode().nodeLists().with(this.key, children));
    }
    ...
    public RuleResult<String> generate(CompoundNode node) {
        ...
        return compoundNodes.stream().map(this.rule::generate).reduce(RuleResult.createFromValue(""), (result, result1) -> result.flatMap(result0 -> result1.mapValue(result2 -> result0 + result2)), (_, next) -> next);
    }
}
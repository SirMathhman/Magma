...
public record StatefulOrRule<T>(Iterable<StatefulRuleAlias<T>> rules) implements StatefulRule<T> {
    public static Tuple2<CompileState, String> compileOrPlaceholder(
            CompileState state,
            String input,
            Iterable<StatefulRuleAlias<String>> rules
    ) {
    ...
    public Option<Tuple2<CompileState, T>> apply(CompileState state, String input) {
        return this.rules.iter()
                .map((StatefulRuleAlias<T> statefulRule) -> statefulRule.apply(state, input))

...
public class StructureCompiler {
    public static Rule<String> createStructureRule(String sourceInfix, String targetInfix) {
        return (CompileState state, String input1) -> new SplitComposable<Tuple2<CompileState, String>>(new LocatingSplitter(sourceInfix, new FirstLocator()), Composable.toComposable((String beforeInfix, String afterInfix) -> new SplitComposable<Tuple2<CompileState, String>>(new LocatingSplitter("{", new FirstLocator()), Composable.toComposable((String beforeContent, String withEnd) -> new SuffixComposable<Tuple2<CompileState, String>>("}", (String inputContent) -> SplitComposable.compileLast(beforeInfix, "\n", (String s, String s2) -> {
            ...
            return StructureCompiler.compileStructureWithImplementing(state, annotations, DefiningCompiler.parseModifiers(s2), targetInfix, beforeContent, inputContent, sourceInfix);
        }).or(() -> {
            var modifiers = DefiningCompiler.parseModifiers(beforeContent);
            return StructureCompiler.compileStructureWithImplementing(state, Lists.empty(), modifiers, targetInfix, beforeContent, inputContent, sourceInfix);
    ...
    public static Option<Tuple2<CompileState, String>> compileStructureWithImplementing(CompileState state, List<String> annotations, List<String> modifiers, String targetInfix, String beforeContent, String content, String sourceInfix) {
        return SplitComposable.compileLast(beforeContent, " implements ", (String s, String s2) -> TypeCompiler.createTypeRule().apply(state, s2).flatMap((Tuple2<CompileState, Type> implementingTuple) -> StructureCompiler.compileStructureWithExtends(implementingTuple.left(), annotations, targetInfix, s, new Some<Type>(implementingTuple.right()), content, sourceInfix))).or(() -> StructureCompiler.compileStructureWithExtends(state, annotations, targetInfix, beforeContent, new None<Type>(), content, sourceInfix));
    }
    ...
    public static Option<Tuple2<CompileState, String>> compileStructureWithExtends(CompileState state, List<String> annotations, String targetInfix, String beforeContent, Option<Type> maybeImplementing, String inputContent, String sourceInfix) {
        ...
        return new SplitComposable<Tuple2<CompileState, String>>(splitter, Composable.toComposable((String beforeExtends, String afterExtends) -> ValueCompiler.values((CompileState inner0, String inner1) -> TypeCompiler.createTypeRule().apply(inner0, inner1)).apply(state, afterExtends)
                .flatMap((Tuple2<CompileState, List<Type>> compileStateListTuple2) -> compileStructureWithParameters(compileStateListTuple2.left(), annotations, targetInfix, beforeExtends, compileStateListTuple2.right(), maybeImplementing, inputContent, sourceInfix)))).apply(beforeContent).or(() -> compileStructureWithParameters(state, annotations, targetInfix, beforeContent, Lists.empty(), maybeImplementing, inputContent, sourceInfix));
    }
    ...
    public static Option<Tuple2<CompileState, String>> compileStructureWithParameters(CompileState state, List<String> annotations, String targetInfix, String beforeContent, Iterable<Type> maybeSuperType, Option<Type> maybeImplementing, String inputContent, String sourceInfix) {
        ...
        return new SplitComposable<Tuple2<CompileState, String>>(splitter1, Composable.toComposable((String rawName, String withParameters) -> {
            ...
            return new SplitComposable<Tuple2<CompileState, String>>(splitter, Composable.toComposable((String parametersString, String _) -> {
                ...
                return compileStructureWithTypeParams(parametersTuple.left(), targetInfix, inputContent, name, parameters, maybeImplementing, annotations, maybeSuperType, sourceInfix);
            })).apply(withParameters);
        })).apply(beforeContent).or(() -> compileStructureWithTypeParams(state, targetInfix, inputContent, beforeContent, Lists.empty(), maybeImplementing, annotations, maybeSuperType, sourceInfix));
    }
    ...
    public static Option<Tuple2<CompileState, String>> compileStructureWithTypeParams(CompileState state, String infix, String content, String beforeParams, Iterable<Definition> parameters, Option<Type> maybeImplementing, List<String> annotations, Iterable<Type> maybeSuperType, String sourceInfix) {
        return new SuffixComposable<Tuple2<CompileState, String>>(">", (String withoutTypeParamEnd) -> {
            ...
            return new SplitComposable<Tuple2<CompileState, String>>(splitter, Composable.toComposable((String name, String typeParamsString) -> {
                ...
                return assembleStructure(state, annotations, infix, name, typeParams, parameters, maybeImplementing, content, maybeSuperType, sourceInfix);
            })).apply(withoutTypeParamEnd);
        }).apply(Strings.strip(beforeParams)).or(() -> assembleStructure(state, annotations, infix, beforeParams, Lists.empty(), parameters, maybeImplementing, content, maybeSuperType, sourceInfix));
    }
    ...
    public static Option<Tuple2<CompileState, String>> assembleStructure(
        ...
        var maybeValuesMethod = generateValuesMethod(sourceInfix, outputContentState, name);

        var extendsString = joinExtends(maybeSuperType);
        var generated = joinedModifiers + targetInfix + name + joinedTypeParams + extendsString + implementingString + " {" + joinParameters(parameters) + constructorString + outputContent + maybeValuesMethod + "\n}\n";
        return new Some<Tuple2<CompileState, String>>(new Tuple2Impl<CompileState, String>(outputContentState.mapRegistry((Registry registry) -> registry.append(generated)), ""));
    }
    ...
    public static String generateValuesMethod(String sourceInfix, CompileState outputContentState, String name) {
        ...
        var joined = mergeEnumValues(outputContentState, name);
        return "\n\tstatic values(): " + name + "[] {\n\t\treturn [" + joined + "];\n\t}";
    }
    ...
    public static String generateConstructorFromRecordParameters(Iterable<Definition> parameters) {
        return parameters.iter()
                ...
                .collect(new Joiner(", "))
                .map((String generatedParameters) -> generateConstructorWithParameterString(parameters, generatedParameters))
                .orElse("");
    }

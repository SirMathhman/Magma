...
public class ArithmeticValidator {
    ...
    public String validateArithmeticExpression(String expression) {
        ...
        if (additionIndex != -1) {
            BinaryOperationParams params = new BinaryOperationParams(
                    expression, additionIndex, "+", "addition");
        ...
        if (subtractionIndex != -1) {
            BinaryOperationParams params = new BinaryOperationParams(
                    expression, subtractionIndex, "-", "subtraction");
            return validateBinaryOperation(params);
        }
        ...
        if (multiplicationIndex != -1) {
            BinaryOperationParams params = new BinaryOperationParams(
                    expression, multiplicationIndex, "*", "multiplication");
            return validateBinaryOperation(params);
        }
    ...
    public String validateBinaryOperation(BinaryOperationParams params) {
        ...
        ArithmeticTypeCheckParams typeParams = new ArithmeticTypeCheckParams(
                leftOperand, rightOperand, leftType, rightType, params.operationName());
    ...
    public boolean isNumericType(String type) {
        return type.equals("I8") || type.equals("I16") || type.equals("I32") || type.equals("I64") || 
               type.equals("U8") || type.equals("U16") || type.equals("U32") || type.equals("U64");
    }
    ...
    public String checkArithmeticTypeConsistency(ArithmeticTypeCheckParams params) {
        ...
        if (params.hasTypeMismatch()) {
            throw new CompileException(params.createTypeMismatchMessage());
        }

        // Return the known type, preferring leftType if both are known
        if (params.leftType() != null) {
            return params.leftType();
        } else {

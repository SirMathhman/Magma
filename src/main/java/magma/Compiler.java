...
public class Compiler {
					...
					if (isCompoundOrIncrement(s)) {
						...
						if (targetCheck != null) {
							...
							if (!numeric) {
								...
								if (usage == 1 || isBracedNumeric(targetCheck.rhs) || isPlainNumeric(targetCheck.rhs)) numeric = true;
							}
							if (!numeric) {
	...
	private void appendVarDeclToBuilder(StringBuilder b, VarDecl d, boolean forC) {
		if (forC) {
			...
			if (d.rhs == null || d.rhs.isEmpty()) {
				...
				if (rhsOut.contains("=>")) rhsOut = cleanArrowRhsForJs(rhsOut);
		...
		if (p.length() >= 2 && p.charAt(0) == '(' && p.charAt(p.length() - 1) == ')') {
			...
			for (String part : parts) {
				...
				if (colon != -1) {
					...
					else if (typ.equals("Bool")) type = "int"; // represent bool as int in C output
	...
	private String[] parseFnDeclaration(String fnDecl) {
		...
		if (parenIdx == -1) return null; // Invalid syntax
	...
	private boolean exprLooksBoolean(String s) {
		...
		if (findStandaloneTokenIndex(t, "true", 0) != -1) return true;
		if (findStandaloneTokenIndex(t, "false", 0) != -1) return true;
	...
	private String exprType(String expr, List<VarDecl> decls) {
		...
		if (isPlainNumeric(s) || isBracedNumeric(s)) return "I32";
		// readInt() call counts as I32
		if (findReadIntUsage(s) == 1) return "I32";

...
class GenericRegistry {
    ...
    static void registerGenericFunction(FunctionRegistration registration) {
        FunctionTemplate template = new FunctionTemplate(registration.functionData.name, registration.functionData.typeParams) {
            {
    ...
    static String monomorphizeStruct(String structName, String typeArg) throws CompileException {
        ...
        if (template == null) {
            throw new CompileException("Generic struct not found: " + structName);
        }
        ...
        if (monomorphizedStructs.containsKey(key)) {
            return monomorphizedStructs.get(key);
        }
    ...
    static String monomorphizeFunction(String functionName, String typeArg) throws CompileException {
        ...
        if (template == null) {
            throw new CompileException("Generic function not found: " + functionName);
        }
        ...
        // Return already monomorphized version if exists
        if (monomorphizedFunctions.containsKey(key)) {
            return monomorphizedFunctions.get(key);
        }
        ...
        String monomorphizedReturnType = template.returnType != null ? template.returnType.replaceAll("\\b" + template.typeParams + "\\b", typeArg) : null;
    ...
    private static String generateFunctionCode(FunctionCodeParams params) throws CompileException {
        ...
        if (params.returnType != null) {
            cReturnType = getCType(params.returnType, params.typeMapping);
        } else {
            // Simple inference - if body contains "return \\d+", assume int32_t
            if (params.body.matches(".*return\\s+\\d+.*")) {
                cReturnType = "int32_t";
            } else if (params.body.matches(".*return\\s+\\w+.*")) {
                // If returning a variable, try to infer from parameter types

...
public class Interpreter {
	public static Result<String, InterpretError> interpret(String input) {
		...
		Option<String> afterAdd = in.flatMap(trimmed -> {
			...
			if (plusIdx <= 0 || plusIdx >= trimmed.length() - 1) return Option.some(trimmed);
			String left = trimmed.substring(0, plusIdx).trim();
			String right = trimmed.substring(plusIdx + 1).trim();
			if (left.isEmpty() || right.isEmpty()) return Option.none();

			Function<String, Option<Num>> parseToken = token -> {
				if (token == null || token.isEmpty()) return Option.none();
				try {
				...
				if (c == '+' || c == '-') i++;
				int ds = i;
				while (i < tlen && Character.isDigit(token.charAt(i))) i++;
				if (i <= ds) return Option.none();
				...
				if (!allowed.contains(suf)) return Option.none();
				if (pref.startsWith("-") && suf.startsWith("U")) return Option.none();
				try {
			...
			if (L.isNone() || R.isNone()) return Option.none();
			Num lnum = L.get();
			Num rnum = R.get();

			String resSuffix = "";
			if (!lnum.suffix.isEmpty() && !rnum.suffix.isEmpty()) {
				if (!lnum.suffix.equals(rnum.suffix)) return Option.none();
		...
		Option<String> finalOpt = afterAdd.flatMap(s -> {
			try {
				...
				if (s.isEmpty()) return Option.none();
				int len = s.length();
				int idx = 0;
				char c = s.charAt(idx);
				if (c == '+' || c == '-') idx++;
				int digitsStart = idx;
				while (idx < len && Character.isDigit(s.charAt(idx))) idx++;
				if (idx > digitsStart) {
					String prefix = s.substring(0, idx);
					String suffix = s.substring(idx);
					var allowed = Set.of("U8", "U16", "U32", "U64", "I8", "I16", "I32", "I64");
					if (suffix.isEmpty()) return Option.some(prefix);
					if (allowed.contains(suffix)) {
						if (prefix.startsWith("-") && suffix.startsWith("U")) return Option.none();

...
public class Interpreter {
	...
	private record LetDecl(boolean isMutable, String id, int valStart, int valEnd, int afterSemicolon) {}

	// helper record for assignment parse result
	private record Assignment(String value, int afterSecondSemicolon) {}

	// helper: parse identifier at pos returning its text and end index
	private record Ident(String id, int start, int end) {}

	// helper: parse a (unsigned) integer after an '=' at pos; returns start/end
	private record NumberSpan(int start, int end) {}

	// helper: number span plus the index after a following semicolon
	private record NumberAndAfter(NumberSpan span, int afterSemicolon) {}

	// helper: small holder for identifier name + end index to reduce repetition
	private record IdentName(String name, int end) {}

	// helper: number span plus the next position after skipping whitespace
	private record NumberSpanWithNext(NumberSpan span, int nextPos) {}
	...
	private static int parseIdentifierStart(String s, int pos) {
		pos = skipWS(s, pos);
		if (pos < s.length() && isIdentifierStartChar(s.charAt(pos))) return pos;
		return -1;
	}

	// helper: if the final identifier at pos matches expectedId and consumes to end
	// return the provided value, otherwise empty
	private static Optional<String> matchFinalIdentifierAndReturnValue(String s,
																																		 int pos,
																																		 String expectedId,
																																		 String value) {
	...
	private static int parseIdentifierEnd(String s, int pos) {
		...
		if (pos < len && isIdentifierStartChar(s.charAt(pos))) {
			do pos++; while (pos < len && (Character.isLetterOrDigit(s.charAt(pos)) || s.charAt(pos) == '_'));
			return pos;
		}
		return -1;
	}
	...
	private static Optional<LetDecl> parseLetDecl(String s) {
		var pos = 0;
		...
		if (idNameOpt.isEmpty()) return Optional.empty();
		IdentName idName = idNameOpt.get();
		String id = idName.name();
		pos = idName.end();
		int afterColon = expectCharAndSkipWS(s, pos, ':');
		if (afterColon == -1) return Optional.empty();
		if (!(afterColon + 3 <= len && s.startsWith("I32", afterColon))) return Optional.empty();
		pos = afterColon + 3;
		Optional<NumberAndAfter> naaOpt = parseNumberAndExpectSemicolon(s, pos);
		if (naaOpt.isEmpty()) return Optional.empty();
	...
	private static Optional<Assignment> parseAssignment(String s, int pos, String expectedId) {
		...
		if (idNameOpt.isEmpty()) return Optional.empty();
		IdentName idName = idNameOpt.get();
		if (!idName.name().equals(expectedId)) return Optional.empty();
	...
	private static Optional<NumberSpan> parseNumberAfterEq(String s, int pos) {
		...
		if (afterEq == -1) return Optional.empty();
		// parse signed number at afterEq (after skipping WS and '=' )
		return parseSignedNumberSpanNoWS(s, afterEq);
	}
	...
	private static Optional<NumberAndAfter> parseNumberAndExpectSemicolon(String s, int pos) {
		...
		if (afterSemicolon == -1) return Optional.empty();
	...
	private static Optional<NumberSpan> parseSignedNumberSpanNoWS(String s, int pos) {
		int len = s.length();
		int cur = pos;
		if (cur < len && (s.charAt(cur) == '+' || s.charAt(cur) == '-')) cur++;
		int end = scanDigits(s, cur);
		if (end > cur) return Optional.of(new NumberSpan(pos, end));
		return Optional.empty();
	}
	...
	private static int scanDigits(String s, int pos) {
		int len = s.length();
		int cur = pos;
		while (cur < len && Character.isDigit(s.charAt(cur))) cur++;
		return cur;
	}

	// helper: does the string start with keyword at pos (followed by whitespace or
	// EOL)
	private static boolean startsWithKeyword(String s, int pos) {
		int len = s.length();
		if (!(len - pos >= "if".length() && s.startsWith("if", pos))) return false;
		int end = pos + "if".length();
	...
	private static int expectKeywordAndSkipWS(String s, int pos, String kw) {
		pos = skipWS(s, pos);
		int len = s.length();
		if (!(len - pos >= kw.length() && s.startsWith(kw, pos))) return -1;
		int end = pos + kw.length();
		if (!(end == len || Character.isWhitespace(s.charAt(end)))) return -1;
		return skipWS(s, end);
	}
	...
	private static Optional<String> parseIfExpression(String s) {
		...
		int len = s.length();
		pos = skipWS(s, pos);
		if (!startsWithKeyword(s, pos)) return Optional.empty();
		pos += 2;
		pos = skipWS(s, pos);
		...
		boolean cond;
		if (pos + 4 <= len && s.startsWith("true", pos)) {
			cond = true;
			pos += 4;
		} else if (pos + 5 <= len && s.startsWith("false", pos)) {
			cond = false;
			pos += 5;
		} else {
			return Optional.empty();
		...
		pos = skipWS(s, pos);
		if (pos >= len || s.charAt(pos) != ')') return Optional.empty();
		pos++;
		pos = skipWS(s, pos);
		// parse first integer
		...
		if (elsep == -1) return Optional.empty();
		pos = elsep;
	...
	public Result<String, InterpretError> interpret(String input) {
		...
		try {
			...
			int firstEnd = parseSignedIntegerEnd(trimmed, pos);
			if (firstEnd != -1) {
				pos = firstEnd;
				pos = skipWS(trimmed, pos);
				if (pos < len) {
					char op = trimmed.charAt(pos);
					if (op == '+' || op == '-' || op == '*' || op == '/') {
						pos++;
						pos = skipWS(trimmed, pos);
						int secondStart = pos;
						int secondEnd = parseSignedIntegerEnd(trimmed, pos);
						if (secondEnd != -1) {
							pos = skipWS(trimmed, secondEnd);
							if (pos == len) {
								try {
									long a = Long.parseLong(trimmed.substring(firstStart, firstEnd));
									long b = Long.parseLong(trimmed.substring(secondStart, secondEnd));
									long res;
									switch (op) {
										case '+':
											res = a + b;
		...
		try {
			Optional<LetDecl> declOpt = parseLetDecl(trimmed);
			if (declOpt.isPresent()) {
				LetDecl decl = declOpt.get();
				if (!decl.isMutable) {
					// immutable: expect a single identifier reference after the semicolon
					Optional<String> maybe = matchFinalIdentifierAndReturnValue(trimmed, decl.afterSemicolon, decl.id,
																																			trimmed.substring(decl.valStart, decl.valEnd));
					if (maybe.isPresent()) return new Ok<>(maybe.get());
				} else {
					// mutable: parse an assignment following the declaration
					Optional<Assignment> asgOpt = parseAssignment(trimmed, decl.afterSemicolon, decl.id);
					if (asgOpt.isPresent()) {
						Assignment asg = asgOpt.get();
						Optional<String> maybe =
								matchFinalIdentifierAndReturnValue(trimmed, asg.afterSecondSemicolon, decl.id, asg.value);
						if (maybe.isPresent()) return new Ok<>(maybe.get());
					}
				}
			}

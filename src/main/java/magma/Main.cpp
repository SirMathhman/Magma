	...
	if (next == '\"') {
		...
		while (true) {
			...
			if (!(maybeTuple.variant = ?.SomeVariant)) 
				break;
			current = value.left;
			/*JMemberAccess[instance=, memberName=right]*/ right = value.right;
			if (right == '\\') 
...
CExpression toCExpression_JMemberAccess(void* _ref){
	JMemberAccess* _this = (JMemberAccess*) _ref;
	/*JExpressionWrapper[content=(*_this).instance.toCExpression()]*/ cExpression = (*_this).instance.toCExpression();
		...
		if (maybePeeked.variant = ?.SomeVariant) {
			...
			while (true) {
				...
				if (maybeTuple.variant = ?.SomeVariant) {
					...
					/*JMemberAccess[instance=, memberName=right]*/ right = tuple.right;
					if (right == '\r' || right == '\n') 

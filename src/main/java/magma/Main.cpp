...
CPrimitiveType CPrimitiveTypeVoid = new_CPrimitiveType(Strings.from("void"))
CPrimitiveType CPrimitiveTypeChar = new_CPrimitiveType(Strings.from("char"))
CPrimitiveType CPrimitiveTypeInt = new_CPrimitiveType(Strings.from("int"))
...
char* from_Strings(void* _ref, char* value)
Main<> new_Main()
...
char* generate_CPointerType(void* _ref){
	CPointerType* _this = (CPointerType*) _ref
	return (*_this).type.generate() + Strings.from("*")
}
char* toBaseName_CPointerType(void* _ref){
	CPointerType* _this = (CPointerType*) _ref
	return (*_this).type.toBaseName() + Strings.from("_ptr")
}
...
char* generate_CTemplateType(void* _ref){
	CTemplateType* _this = (CTemplateType*) _ref
	/*(*_this).list.iter().map(F? { alloc(CType), F?Table { generate }}).collect(new_Joiner(Strings.from(", ")))*/ typeArguments = (*_this).list.iter().map(F? { alloc(CType), F?Table { generate }}).collect(new_Joiner(Strings.from(", ")))
	return (*_this).base + Strings.from(" < ") + typeArguments + Strings.from(">")
}
char* toBaseName_CTemplateType(void* _ref){
	CTemplateType* _this = (CTemplateType*) _ref
	return (*_this).base
...
char* generate_F1RDeclaration(void* _ref){
	F1RDeclaration* _this = (F1RDeclaration*) _ref
	/*Strings.from("(") + this.parameterTypes.iter().map(CType::generate).collect(new Joiner(Strings.from(", "))) +
					Strings.from(")")*/ joinedParameterTypes = Strings.from("(") + this.parameterTypes.iter().map(CType::generate).collect(new Joiner(Strings.from(", "))) +
					Strings.from(")")
	return (*_this).type.generate() + Strings.from(" (*") + this.name + Strings.from(")") + joinedParameterTypes
}
...
char* generate_CDeclaration(void* _ref){
	...
	return template + (*_this).type.generate() + Strings.from(" ") + (*_this).name
}
...
auto lambda8(void* _ref, auto typeParam){
	return Strings.from("typename ") + typeParam
}
char* generateTemplateString_Main(void* _ref, List<char*> typeParameters){
	Main* _this = (Main*) _ref
	char* templateString
	if (typeParameters.isEmpty()) 
		templateString = Strings.from("")
	else {
		/*typeParameters.iter().map(lambda8).collect(new_Joiner(Strings.from(", ")))*/ typeNames = typeParameters.iter().map(lambda8).collect(new_Joiner(Strings.from(", ")))
...
char* wrap_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	/*input.replace(Strings.from("start"), Strings.from("start")).replace(Strings.from("end"), Strings.from("end"))*/ replaced = input.replace(Strings.from("/*"), Strings.from("start")).replace(Strings.from("*/"), Strings.from("end"))
	return Strings.from("/*") + replaced + Strings.from("*/")
}
void main_Main(void* _ref, char** args){
	Main* _this = (Main*) _ref
...
char* generateStatement_Main(void* _ref, int depth, char* content){
	Main* _this = (Main*) _ref
	return generateIndent(depth) + content + Strings.from("")
}
char* generateIndent_Main(void* _ref, int depth){
	Main* _this = (Main*) _ref
...
Option<IOError> run_Main(void* _ref){
	Main* _this = (Main*) _ref
	/*Paths.get(Strings.from("."), Strings.from("src"), Strings.from("main"), Strings.from("java"), Strings.from("magma"), Strings.from("Main.java"))*/ source = Paths.get(Strings.from("."), Strings.from("src"), Strings.from("main"), Strings.from("java"), Strings.from("magma"), Strings.from("Main.java"))
	/*source.resolveSibling(Strings.from("Main.cpp"))*/ target = source.resolveSibling(Strings.from("Main.cpp"))
...
char* compile_Main(void* _ref, char* input){
	...
	/*(*_this).joinStrings(Strings.from(""), (*_this).structures)*/ joinedStructures = (*_this).joinStrings(Strings.from(""), (*_this).structures)
	/*(*_this).joinStrings(Strings.from(""), (*_this).globals)*/ joinedGlobals = (*_this).joinStrings(Strings.from(""), (*_this).globals)
	/*(*_this).joinStrings(Strings.from(""), (*_this).functionDeclarations)*/ joinedFunctionDeclarations = (*_this).joinStrings(Strings.from(""), (*_this).functionDeclarations)
	/*(*_this).joinStrings(Strings.from(""), (*_this).functions)*/ joinedFunctions = (*_this).joinStrings(Strings.from(""), (*_this).functions)
...
char* compileAll_Main(void* _ref, char* input, F1R<char*, char*> mapper, Folder folder){
	Main* _this = (Main*) _ref
	return (*_this).divide(input, folder).map(mapper).collect(new_Joiner(Strings.from("")))
}
Stream<char*> divide_Main(void* _ref, char* input, Folder folder){
	Main* _this = (Main*) _ref
...
char* compileRootSegment_Main(void* _ref, char* input){
	...
	if (stripped.isEmpty()) 
		return Strings.from("")
	if (stripped.startsWith(Strings.from("package ")) || stripped.startsWith(Strings.from("import "))) 
		return Strings.from("")
	return (*_this).compileStructure(Strings.from("class"), stripped).map(F? { alloc(CStructMember), F?Table { generate }}).orElseGet(lambda9)
}
...
auto lambda17(void* _ref, auto variant){
	return System.lineSeparator() + Strings.from("\t") + variant + Strings.from("Variant")
}
auto lambda18(void* _ref, auto variant){
	return System.lineSeparator() + Strings.from("\t") + variant + joinedTypeParameters + Strings.from(" ") + variant + Strings.from("")
}
...
Option<CStructMember> compileStructure_Main(void* _ref, char* type, char* stripped){
	Main* _this = (Main*) _ref
	/*stripped.indexOf(type + Strings.from(" "))*/ i = stripped.indexOf(type + Strings.from(" "))
	if (i < 0) 
	...
	/*beforeType.lastIndexOf(Strings.from("\n"))*/ i5 = beforeType.lastIndexOf(Strings.from("\n"))
	if (i5 >= 0) {
	...
	if (annotations.contains(Strings.from("Actual"))) 
		return new_Some<CStructMember>(new_EmptyStructMember())
	/*stripped.substring(i + (type + Strings.from(" ")).length()).strip()*/ afterKeyword = stripped.substring(i + (type + Strings.from(" ")).length()).strip()
	/*afterKeyword.indexOf(Strings.from("{"))*/ i1 = afterKeyword.indexOf(Strings.from("{"))
	if (i1 < 0) 
		return new_None<CStructMember>()
	...
	if (!withEnd.endsWith(Strings.from("}"))) 
		return new_None<CStructMember>()
	/*withEnd.substring(0, withEnd.length() - 1)*/ inputContent = withEnd.substring(0, withEnd.length() - 1)
	List<char*> variants = Lists.empty()
	/*beforeContent.indexOf(Strings.from("permits "))*/ i2 = beforeContent.indexOf(Strings.from("permits "))
	if (i2 >= 0) {
		/*beforeContent.substring(i2 + Strings.from("permits ").length())*/ substring1 = beforeContent.substring(i2 + Strings.from("permits ").length())
		beforeContent = beforeContent.substring(0, i2)
		variants = (*_this).splitValues(substring1)
	}
	List<CType> implementees = Lists.empty()
	/*beforeContent.indexOf(Strings.from("implements "))*/ i4 = beforeContent.indexOf(Strings.from("implements "))
	if (i4 >= 0) {
		/*beforeContent.substring(i4 + Strings.from("implements ").length())*/ implementeesString = beforeContent.substring(i4 + Strings.from("implements ").length())
	...
	/*beforeContent.indexOf(Strings.from(" < "))*/ i3 = beforeContent.indexOf(Strings.from(" < "))
	if (i3 >= 0) {
		/*beforeContent.substring(i3 + 1).strip()*/ substring1 = beforeContent.substring(i3 + 1).strip()
		if (substring1.endsWith(Strings.from(">"))) {
	...
	if (!(*_this).isIdentifier(beforeContent)) 
		return new_None<CStructMember>()
	/*Streams.fromObjArray(modifiers.split(Pattern.quote(Strings.from(" ")))).map(F? { alloc(String), F?Table { strip }}).filter(lambda14).toList()*/ modifiersList = Streams.fromObjArray(modifiers.split(Pattern.quote(Strings.from(" ")))).map(F? { alloc(String), F?Table { strip }}).filter(lambda14).toList()
	...
	if (modifiersList.contains(Strings.from("sealed"))) {
		/*variants.iter().map(lambda17).collect(new_Joiner(Strings.from(",")))*/ enumFields = variants.iter().map(lambda17).collect(new_Joiner(Strings.from(",")))
		/*Strings.from("enum ") + name + Strings.from("Variant {") + enumFields + System.lineSeparator() + Strings.from("}") + System.lineSeparator()*/ generatedEnum = Strings.from("enum ") + name + Strings.from("Variant {") + enumFields + System.lineSeparator() + Strings.from("}") + System.lineSeparator()
		/*variants.iter().map(lambda18).collect(new_Joiner())*/ unionFields = variants.iter().map(lambda18).collect(new_Joiner())
		/*templateString + Strings.from("union ") + name + Strings.from("Data {") + unionFields + System.lineSeparator() + Strings.from("}") + System.lineSeparator()*/ generatedUnion = templateString + Strings.from("union ") + name + Strings.from("Data {") + unionFields + System.lineSeparator() + Strings.from("}") + System.lineSeparator()
		/*name + Strings.from("Variant variant")*/ s = name + Strings.from("Variant variant")
		/*name + Strings.from("Data") + joinedTypeParameters + Strings.from(" data")*/ s1 = name + Strings.from("Data") + joinedTypeParameters + Strings.from(" data")
	...
	if (type.equals(Strings.from("interface"))) {
		/*(*_this).generateStatement(name + Strings.from("Table") + joinedTypeParameters + Strings.from(" table"))*/ table = (*_this).generateStatement(name + Strings.from("Table") + joinedTypeParameters + Strings.from(" table"))
		/*(*_this).generateStatement(Strings.from("void* data"))*/ data = (*_this).generateStatement(Strings.from("void* data"))
		/*members.iter().map(F? { alloc(CStructMember), F?Table { generate }}).map(F? { alloc((*_this)), F?Table { generateStatement }}).collect(new_Joiner(Strings.from("")))*/ tableMembers = members.iter().map(F? { alloc(CStructMember), F?Table { generate }}).map(F? { alloc((*_this)), F?Table { generateStatement }}).collect(new_Joiner(Strings.from("")))
		/*templateString + Strings.from("struct ") + name + Strings.from("Table {") + tableMembers + System.lineSeparator() + Strings.from("}") + System.lineSeparator()*/ vTable = templateString + Strings.from("struct ") + name + Strings.from("Table {") + tableMembers + System.lineSeparator() + Strings.from("}") + System.lineSeparator()
	...
	/*dependencies + templateString + Strings.from("struct ") + name + Strings.from(" {") + joinedRecordFields + fields + System.lineSeparator() + Strings.from("}") + System.lineSeparator()*/ generated = dependencies + templateString + Strings.from("struct ") + name + Strings.from(" {") + joinedRecordFields + fields + System.lineSeparator() + Strings.from("}") + System.lineSeparator()
...
char* getString_Main(void* _ref, CType implementee, char* name, char* joinedTypeParameters, char* templateString){
	...
	/*(*_this).generateStatement(thisType + Strings.from(" _this = *((") + thisType + Strings.from("*) _ref)"))*/ s = (*_this).generateStatement(thisType + Strings.from(" _this = *((") + thisType + Strings.from("*) _ref)"))
	/*(*_this).generateStatement(identifier + Strings.from("Data") + joinedTypeParameters + Strings.from(" data"))*/ s1 = (*_this).generateStatement(identifier + Strings.from("Data") + joinedTypeParameters + Strings.from(" data"))
	/*(*_this).generateStatement(Strings.from("data.") + name + Strings.from(" = _this"))*/ s2 = (*_this).generateStatement(Strings.from("data.") + name + Strings.from(" = _this"))
	/*(*_this).generateStatement(Strings.from("return { ") + name + Strings.from("Variant, data }"))*/ s3 = (*_this).generateStatement(Strings.from("return { ") + name + Strings.from("Variant, data }"))
	/*s + s1 + s2 + s3*/ conversionF1RContent = s + s1 + s2 + s3
	return templateString + implementee.generate() + Strings.from(" to") + identifier + Strings.from("_") + name + Strings.from("(void* _ref){") + conversionF1RContent + System.lineSeparator() + Strings.from("}") + System.lineSeparator()
}
char* joinTypeParameters_Main(void* _ref, List<char*> typeParameters){
	Main* _this = (Main*) _ref
	char* joinedTypeParameters
	if (typeParameters.isEmpty()) 
		joinedTypeParameters = Strings.from("")
	else joinedTypeParameters = Strings.from(" < ") + typeParameters.iter().collect(new_Joiner(Strings.from(", "))) + Strings.from(">")
...
List<char*> splitValues_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	/*input.split(Pattern.quote(Strings.from(",")))*/ segments = input.split(Pattern.quote(Strings.from(",")))
...
Option<CStructMember> compileClassSegment_Main(void* _ref, char* input, char* structName, List<char*> typeParameters, List<char*> variants){
	...
	if (stripped.isEmpty()) 
		return new_None<CStructMember>()
	/*(*_this).compileStructure(Strings.from("enum"), input)*/ maybeEnum = (*_this).compileStructure(Strings.from("enum"), input)
	if (maybeEnum.variant = ?.SomeVariant) 
		return maybeEnum
	/*(*_this).compileStructure(Strings.from("interface"), input)*/ maybeInterface = (*_this).compileStructure(Strings.from("interface"), input)
	if (maybeInterface.variant = ?.SomeVariant) 
		return maybeInterface
	/*(*_this).compileStructure(Strings.from("record"), input)*/ maybeRecord = (*_this).compileStructure(Strings.from("record"), input)
	if (maybeRecord.variant = ?.SomeVariant) 
		return maybeRecord
	/*(*_this).compileStructure(Strings.from("class"), input)*/ maybeClass = (*_this).compileStructure(Strings.from("class"), input)
...
auto lambda24(void* _ref, auto name){
	return name + Strings.from("_") + structName
}
...
auto lambda27(void* _ref){
	if (variants.isEmpty()) {
		/*finalParameters.subList(1, finalParameters.size()).iter().map(lambda25).toList().addFirst(Strings.from("_this->data")).iter().collect(new_Joiner(Strings.from(", ")))*/ joinedParameters = finalParameters.subList(1, finalParameters.size()).iter().map(lambda25).toList().addFirst(Strings.from("_this->data")).iter().collect(new_Joiner(Strings.from(", ")))
		return (*_this).generateStatement(Strings.from("return _this->table.") + declaration.name + Strings.from("(") + joinedParameters +
							Strings.from(")"))
	}
	else {
		/*(*_this).generateStatement(transformType(declaration.type).generate() + Strings.from(" _ret"))*/ returnValueDefinition = (*_this).generateStatement(transformType(declaration.type).generate() + Strings.from(" _ret"))
		/*variants.iter().map(lambda26).collect(new_Joiner())*/ cases = variants.iter().map(lambda26).collect(new_Joiner())
		return returnValueDefinition + generateIndent(1) + Strings.from("switch (") + Strings.from("_this->variant"
					) +
								 Strings.from(") {") + cases + generateIndent(1) + Strings.from("}") + (*_this).generateStatement(Strings.from("return _ret"))
	}
}
...
Option<CStructMember> compileMethod_Main(void* _ref, char* structName, List<char*> typeParameters, List<char*> variants, char* input){
	Main* _this = (Main*) _ref
	/*input.indexOf(Strings.from("("))*/ i = input.indexOf(Strings.from("("))
	if (i < 0) 
		return new_None<CStructMember>()
	/*input.substring(0, i)*/ declarationString = input.substring(0, i)
	/*input.substring(i + 1)*/ substring1 = input.substring(i + 1)
	/*substring1.indexOf(Strings.from(")"))*/ i1 = substring1.indexOf(Strings.from(")"))
	if (i1 < 0) 
		return new_None<CStructMember>()
	...
	if (methodDeclaration.variant = ?.JDeclaration declaration &&
				declaration.annotations.contains(Strings.from("Actual"))Variant) {
		/*parameters.iter().map(F? { alloc(CDeclaration), F?Table { generate }}).collect(new_Joiner(Strings.from(", ")))*/ compiledParameters = parameters.iter().map(F? { alloc(CDeclaration), F?Table { generate }}).collect(new_Joiner(Strings.from(", ")))
		/*declaration.mapName(lambda24).toCDeclaration()*/ modifiedMethodDeclaration = declaration.mapName(lambda24).toCDeclaration()
		(*_this).functionDeclarations = (*_this).functionDeclarations.addLast(modifiedMethodDeclaration.generate() + Strings.from("(") + compiledParameters + Strings.from(")") + System.lineSeparator())
		return new_Some<CStructMember>(new_EmptyStructMember())
	}
	if (withBraces.startsWith(Strings.from("{")) && withBraces.endsWith(Strings.from("}"))) {
	...
	if (methodDeclaration.variant = ?.JConstructorVariant) {
		/*maybeCompiled.orElse(Strings.from("?"))*/ compiled = maybeCompiled.orElse(Strings.from("?"))
		outputContent = (*_this).generateStatement(structName + Strings.from(" _this")) + compiled + (*_this).generateStatement(Strings.from("return ") + Strings.from("_this"))
	}
	else 
	if (methodDeclaration.variant = ?.JDeclaration declarationVariant) {
		parameters = parameters.addFirst(new_CDeclaration(new_CPointerType(CPrimitiveType.Void), Strings.from("_ref")))
		/*(*_this).joinTypeParameters(typeParameters)*/ joinedTypeParameters = (*_this).joinTypeParameters(typeParameters)
		/*(*_this).generateStatement(structName + joinedTypeParameters + Strings.from("* _this = (") + structName + joinedTypeParameters +
					Strings.from("*) _ref"))*/ thisInitialization = (*_this).generateStatement(structName + joinedTypeParameters + Strings.from("* _this = (") + structName + joinedTypeParameters +
					Strings.from("*) _ref"))
		/*parameters*/ finalParameters = parameters
		outputContent = thisInitialization + maybeCompiled.orElseGet(lambda27)
	}
	else outputContent = Strings.from("?")
	/*parameters.iter().map(F? { alloc(CDeclaration), F?Table { generate }}).collect(new_Joiner(Strings.from(", ")))*/ compiledParameters = parameters.iter().map(F? { alloc(CDeclaration), F?Table { generate }}).collect(new_Joiner(Strings.from(", ")))
	...
	/*mapped.generate() + Strings.from("(") + compiledParameters + Strings.from(")")*/ header = mapped.generate() + Strings.from("(") + compiledParameters + Strings.from(")")
	/*header + Strings.from("{") + outputContent + System.lineSeparator() + Strings.from("}") + System.lineSeparator()*/ generated = header + Strings.from("{") + outputContent + System.lineSeparator() + Strings.from("}") + System.lineSeparator()
	(*_this).functionDeclarations = (*_this).functionDeclarations.addLast(header + Strings.from("") + System.lineSeparator())
...
char* generateCase_Main(void* _ref, JDeclaration declaration, char* variant){
	Main* _this = (Main*) _ref
	return generateIndent(2) + Strings.from("case ") + variant + Strings.from("Variant:") + generateStatement(3, Strings.from("_ret = ") + declaration.name + Strings.from("_") + variant + Strings.from("(&(_this->data.") +
																																																							variant +
																																																							Strings.from(
																																																									"))")) + generateStatement(3, Strings.from("break"))
}
...
Option<JMethodDeclaration> parseConstructor_Main(void* _ref, char* declaration, char* structName){
	...
	/*stripped.lastIndexOf(Strings.from(" "))*/ i = stripped.lastIndexOf(Strings.from(" "))
	if (i >= 0) {
...
Option<CStructMember> compileEnumValue_Main(void* _ref, char* structName, char* enumValue){
	Main* _this = (Main*) _ref
	if (enumValue.endsWith(Strings.from(")"))) {
		/*enumValue.substring(0, enumValue.length() - 1)*/ substring = enumValue.substring(0, enumValue.length() - 1)
		/*substring.indexOf(Strings.from("("))*/ i = substring.indexOf(Strings.from("("))
		if (i >= 0) {
			...
			/*structName + Strings.from(" ") + structName + name + Strings.from(" = ") + Strings.from("new_") + structName + Strings.from("(") + substring2 + Strings.from(")") + Strings.from("") + System.lineSeparator()*/ generated = structName + Strings.from(" ") + structName + name + Strings.from(" = ") + Strings.from("new_") + structName + Strings.from("(") + substring2 + Strings.from(")") + Strings.from("") + System.lineSeparator()
...
char* compileMethodSegment_Main(void* _ref, char* input, int indent){
	...
	if (stripped.isEmpty()) 
		return Strings.from("")
	/*(*_this).compileConditional(Strings.from("if"), indent, stripped)*/ maybeIf = (*_this).compileConditional(Strings.from("if"), indent, stripped)
	if (maybeIf.variant = ?.SomeVariant) 
		return result
	/*(*_this).compileConditional(Strings.from("while"), indent, stripped)*/ maybeWhile = (*_this).compileConditional(Strings.from("while"), indent, stripped)
	if (maybeWhile.variant = ?.SomeVariant) 
		return result
	if (stripped.endsWith(Strings.from(";"))) {
		/*stripped.substring(0, stripped.length() - 1)*/ substring = stripped.substring(0, stripped.length() - 1)
		return generateIndent(indent) + (*_this).compileMethodStatement(substring) + Strings.from("")
	}
	if (stripped.startsWith(Strings.from("else "))) {
		/*stripped.substring(Strings.from("else ").length()).strip()*/ substring = stripped.substring(Strings.from("else ").length()).strip()
		if (substring.startsWith(Strings.from("{")) && substring.endsWith(Strings.from("}"))) {
			/*substring.substring(1, substring.length() - 1)*/ substring1 = substring.substring(1, substring.length() - 1)
			return generateIndent(indent) + Strings.from("else {") + (*_this).compileMethodsSegments(substring1, indent + 1) + generateIndent(indent) + Strings.from("}")
		}
...
Option<char*> compileConditional_Main(void* _ref, char* type, int indent, char* input){
	...
	if (input.startsWith(type)) {
		...
		if (substring.startsWith(Strings.from("("))) {
			...
			/*(*_this).joinStrings(Strings.from(""), divisions.subList(1, divisions.size()))*/ maybeWithBraces = (*_this).joinStrings(Strings.from(""), divisions.subList(1, divisions.size()))
			if (!first.endsWith(Strings.from(")"))) 
				return new_None<char*>()
			/*first.substring(0, first.length() - 1)*/ condition = first.substring(0, first.length() - 1)
			if (maybeWithBraces.startsWith(Strings.from("{")) && maybeWithBraces.endsWith(Strings.from("}"))) {
				/*maybeWithBraces.substring(1, maybeWithBraces.length() - 1)*/ content = maybeWithBraces.substring(1, maybeWithBraces.length() - 1)
				return new_Some<char*>(generateIndent(indent) + type + Strings.from(" (") + this.compileExpressionOrPlaceholder(condition) +
							Strings.from(") {") + (*_this).compileMethodsSegments(content, indent + 1) + generateIndent(indent) + Strings.from("}"))
			}
			return new_Some<char*>(generateIndent(indent) + type + Strings.from(" (") + this.compileExpressionOrPlaceholder(condition) +
						Strings.from(") ") + (*_this).compileMethodSegment(maybeWithBraces, indent + 1))
		}
	}
	return new_None<char*>()
}
char* compileMethodStatement_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	/*input.strip()*/ stripped = input.strip()
	if (stripped.equals(Strings.from("break"))) 
		return Strings.from("break")
	if (stripped.startsWith(Strings.from("return "))) 
		return Strings.from("return ") + (*_this).compileExpressionOrPlaceholder(stripped.substring(Strings.from("return ").length()))
	...
	if (maybeInvokable.variant = ?.SomeVariant) 
		return value
	/*(*_this).post(stripped, Strings.from("++"))*/ instance = (*_this).post(stripped, Strings.from("++"))
	if (instance.variant = ?.SomeVariant) 
		return x
	/*(*_this).post(stripped, Strings.from("--"))*/ instance0 = (*_this).post(stripped, Strings.from("--"))
...
Option<char*> compileAssignment_Main(void* _ref, char* stripped){
	Main* _this = (Main*) _ref
	/*stripped.indexOf(Strings.from("="))*/ index = stripped.indexOf(Strings.from("="))
	if (index >= 0) {
		...
		if (maybeSource.variant = ?.SomeVariant) 
			return new_Some<char*>((*_this).transformAssignable(assignable, source).generate() + Strings.from(" = ") + source.generate())
	}
	return new_None<char*>()
}
CAssignable transformAssignable_Main(void* _ref, JAssignable assignable, CExpression source){
...
auto lambda56(void* _ref, auto ()){
	return (*_this).compileOperator(stripped, Strings.from(" >= "))
}
...
Option<char*> getStringOption_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	/*input.strip()*/ stripped = input.strip()
	if (stripped.equals(Strings.from("this"))) 
		return new_Some<char*>(Strings.from("(*_this)"))
	if (stripped.startsWith(Strings.from("switch "))) 
		return new_Some<char*>(Strings.from("_switch"))
	/*stripped.lastIndexOf(Strings.from("::"))*/ i2 = stripped.lastIndexOf(Strings.from("::"))
	if (i2 >= 0) {
		...
		if ((*_this).isIdentifier(name)) {
			...
			/*Strings.from("F?")*/ functionalInterfaceName = Strings.from("F?")
			return new_Some<char*>(functionalInterfaceName + Strings.from(" { alloc(") + compiled + Strings.from("), ") + functionalInterfaceName + Strings.from("Table { ") + name + Strings.from(" }}"))
		}
	}
	if (stripped.startsWith(Strings.from("'")) && stripped.endsWith(Strings.from("'"))) 
	...
	if (maybeLambda.variant = ?.SomeVariant) 
		return maybeLambda
	/*stripped.indexOf(Strings.from(".variant = ?."Variant))*/ i3 = stripped.indexOf(Strings.from(".variant = ?."Variant))
	if (i3 >= 0) {
		/*stripped.substring(0, i3)*/ substring = stripped.substring(0, i3)
		/*stripped.substring(i3 + Strings.from(".variant = ?."Variant).length()).strip()*/ substring1 = stripped.substring(i3 + Strings.from(".variant = ?."Variant).length()).strip()
		...
		if (maybeInstance.variant = ?.SomeVariant) {
			/*substring1.indexOf(Strings.from(" < "))*/ i4 = substring1.indexOf(Strings.from(" < "))
			char* substring2
			if (i4 >= 0) 
				substring2 = substring1.substring(0, i4)
			else substring2 = substring1
			return new_Some<char*>(instance + Strings.from(".variant = ?.") + substring2 + Strings.from("Variant"))
		}
	}
	/*stripped.lastIndexOf(Strings.from("."))*/ i = stripped.lastIndexOf(Strings.from("."))
	if (i >= 0) {
		...
		if ((*_this).isIdentifier(memberName)) {
			...
			if (maybeInstance.variant = ?.SomeVariant) {
				...
				instance = value
				char* generated
				if (instance.equals(Strings.from("this"))) 
					generated = Strings.from("_this->") + memberName
	...
	/*(*_this).compileOperator(stripped, Strings.from(" == ")).or(lambda62).or(lambda61).or(lambda60).or(lambda59).or(lambda58).or(lambda57).or(lambda56)*/ maybeOperator = (*_this).compileOperator(stripped, Strings.from(" == ")).or(lambda62).or(lambda61).or(lambda60).or(lambda59).or(lambda58).or(lambda57).or(lambda56)
	...
	if (stripped.startsWith(Strings.from("!"))) {
		...
		/*(*_this).parseCExpression(substring).map(F? { alloc(CExpression), F?Table { generate }})*/ maybeInstance = (*_this).parseCExpression(substring).map(F? { alloc(CExpression), F?Table { generate }})
		if (maybeInstance.variant = ?.SomeVariant) 
			return new_Some<char*>(Strings.from("!") + instance)
	}
...
auto lambda64(void* _ref, auto param){
	return Strings.from("auto ") + param
}
Option<char*> compileLambda_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	/*input.indexOf(Strings.from("->"))*/ index = input.indexOf(Strings.from("->"))
	if (index < 0) 
		return new_None<char*>()
	/*input.substring(0, index).strip()*/ beforeContent = input.substring(0, index).strip()
	/*input.substring(index + 2).strip()*/ maybeWithBraces = input.substring(index + 2).strip()
	List<char*> params
	if ((*_this).isIdentifier(beforeContent)) 
		params = Lists.of(beforeContent)
	else 
	if (beforeContent.startsWith(Strings.from("(")) && beforeContent.endsWith(/*Strings.from(")"*/))) {
	...
	if (maybeWithBraces.startsWith(Strings.from("{")) && maybeWithBraces.endsWith(Strings.from("}"))) {
		...
		/*params.iter().map(lambda64).toList().addFirst(Strings.from("void* _ref"))*/ paramList = params.iter().map(lambda64).toList().addFirst(Strings.from("void* _ref"))
		/*(*_this).joinStrings(Strings.from(", "), paramList)*/ joined = (*_this).joinStrings(Strings.from(", "), paramList)
		(*_this).functions = (*_this).functions.addLast(Strings.from("auto ") + generatedName + Strings.from("(") + joined + Strings.from("){") + compiled + System.lineSeparator() + Strings.from("}") + System.lineSeparator())
		return new_Some<char*>(generatedName)
	}
	else {
		/*(*_this).generateName()*/ generatedName = (*_this).generateName()
		(*_this).functions = (*_this).functions.addLast(Strings.from("auto ") + generatedName + Strings.from("(void* _ref, auto ") + beforeContent +
					Strings.from(")") + Strings.from("{") + (*_this).generateStatement(Strings.from("return ") + (*_this).compileExpressionOrPlaceholder(maybeWithBraces)) + System.lineSeparator() + Strings.from("}") + System.lineSeparator())
		return new_Some<char*>(generatedName)
	}
}
char* generateName_Main(void* _ref){
	Main* _this = (Main*) _ref
	/*Strings.from("lambda") + (*_this).counter*/ generatedName = Strings.from("lambda") + (*_this).counter
	(*_this).counter++
	...
	if (i1 >= 0) {
		...
		if ((*_this).parseCExpression(leftString).map(F? { alloc(CExpression), F?Table { generate }}).variant = ?.SomeVariant) 
			if ((*_this).parseCExpression(right).map(F? { alloc(CExpression), F?Table { generate }}).variant = ?.SomeVariant) 
				return new_Some<char*>(leftCompiled + Strings.from(" ") + operator + Strings.from(" ") + rightCompiled)
	}
	return new_None<char*>()
}
Option<char*> compileInvokable_Main(void* _ref, char* stripped){
	...
	if (stripped.endsWith(Strings.from(")"))) {
		...
		if (callerStart >= 0) {
			...
			/*(*_this).divide(arguments, new_EscapedFolder(new_ValueFolder())).map(F? { alloc((*_this)), F?Table { compileExpressionOrPlaceholder }}).collect(new_Joiner(Strings.from(", ")))*/ joinedArguments = (*_this).divide(arguments, new_EscapedFolder(new_ValueFolder())).map(F? { alloc((*_this)), F?Table { compileExpressionOrPlaceholder }}).collect(new_Joiner(Strings.from(", ")))
			/*(*_this).compileCaller(callerString)*/ maybeCaller = (*_this).compileCaller(callerString)
			if (maybeCaller.variant = ?.SomeVariant) 
				return new_Some<char*>(value + Strings.from("(") + joinedArguments + Strings.from(")"))
		}
	}
	return new_None<char*>()
}
...
int isNumber_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	if (input.startsWith(Strings.from(" - "))) 
...
Option<char*> compileCaller_Main(void* _ref, char* input){
	...
	if (stripped.startsWith(Strings.from("new "))) {
		/*stripped.substring(Strings.from("new ").length())*/ type = stripped.substring(Strings.from("new ").length())
		return new_Some<char*>(Strings.from("new_") + (*_this).compileType(type))
	}
	return new_None<char*>()
}
Option<JDeclaration> parseDeclaration_Main(void* _ref, char* input){
	Main* _this = (Main*) _ref
	/*input.strip()*/ stripped = input.strip()
	/*stripped.lastIndexOf(Strings.from(" "))*/ nameSeparator = stripped.lastIndexOf(Strings.from(" "))
	if (nameSeparator >= 0) {
		...
		if (beforeType.endsWith(Strings.from(">"))) {
		...
		List<char*> annotations = Lists.empty()
		/*beforeType.lastIndexOf(Strings.from("\n"))*/ i = beforeType.lastIndexOf(Strings.from("\n"))
		if (i >= 0) {

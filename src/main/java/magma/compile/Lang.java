...
public class Lang {
	...
	public sealed interface JStructureSegment
			permits BlockComment, Field, Invalid, JDefinition, JInitialization, JStructure, LineComment, JMethod,
			Whitespace {}

	sealed public interface JExpression
			permits And, JNodes.JCast, CharNode, Identifier, Index, InstanceOf, Invalid, JAdd, JConstruction, JEquals,
			JFieldAccess, JGreaterThan, JGreaterThanEquals, JInvocation, JLessThan, JLessThanEquals, JNotEquals, JOr, JString,
			JSubtract, Lambda, MethodAccess, NewArray, Not, NumberNode, Quantity, SwitchExpr {}
	...
	public record JMethod(JDefinition definition, Option<NonEmptyList<JDefinition>> params,
												Option<NonEmptyList<JMethodSegment>> body, Option<NonEmptyList<Identifier>> typeParameters)
	...
	@Tag("invocation")
	public record CInvocation(CExpression caller, Option<NonEmptyList<CExpression>> arguments)
			implements CFunctionSegment, CExpression {}
	...
	private record OperatorFolder(Slice operator) implements Folder {
		...
		public DivideState fold(DivideState state, char c) {
			if (c == operator.value().charAt(0)) {
				if (operator.value().length() > 1 && state.peek() instanceof Some<Character>(Character next) &&
						next == operator.value().charAt(1)) {
					state.pop();
	...
	private static Rule CTemplate(LazyRule rule) {
		Rule base = String("base");
		final Rule arguments = Or(Expressions("typeArguments", rule));
		return Tag("template", Strip(Suffix(First(base, "<", arguments), ">")));
	}

...
Result</*???*/, /*???*/> serializeOptionField_JavaSerializer(/*???*/ component, /*???*/ value) {
	...
	/*???*/ elementType=/*???*/.value();
	Result<Class</*???*/>, /*???*/> elementClassResult=erase(elementType);
	if (/*???*/)return new_???(error);
...
Result<List</*???*/>, /*???*/> serializeListElements_JavaSerializer(Class</*???*/> elementClass, List</*???*/> list) {
	/*???*/=new_???();
	/*???*/=new_???();
...
Result</*???*/, /*???*/> deserializeSealed_JavaSerializer(Class</*???*/> type, /*???*/ node) {
	...
	Option<Result</*???*/, /*???*/>> directResult=tryDirectPermittedSubclasses(type, node, nodeType);
	if (/*???*/)/*???*/ result;
	return tryNestedSealedInterfaces(type, node, nodeType);
}
Option<Result</*???*/, /*???*/>> tryDirectPermittedSubclasses_JavaSerializer(Class</*???*/> type, /*???*/ node, /*???*/ nodeType) {
...
Result</*???*/, /*???*/> tryNestedSealedInterfaces_JavaSerializer(Class</*???*/> type, /*???*/ node, /*???*/ nodeType) {
	Option<Result</*???*/, /*???*/>> recursiveResult=findNestedSealedDeserialization(type, node, nodeType);
	if (/*???*/)/*???*/ value;
	/*???*/(type);
	/*???*/ validTagsList;
...
Option<Result</*???*/, /*???*/>> findNestedSealedDeserialization_JavaSerializer(Class</*???*/> type, /*???*/ node, /*???*/ nodeType) {
	Class</*???*/>* subclasses=type.getPermittedSubclasses();
	/*???*/ j=/*???*/;
	while (/*???*/)
	{
	Class</*???*/> permitted=/*???*/;
	Option<Result</*???*/, /*???*/>> recursiveResult=tryDeserializeNestedSealed(type, node, nodeType, permitted);
	if (/*???*/)/*???*/ k;
	j++;}
	return new_???();
}
Option<Result</*???*/, /*???*/>> tryDeserializeNestedSealed_JavaSerializer(Class</*???*/> type, /*???*/ node, /*???*/ nodeType, Class</*???*/> permitted) {
	if (/*???*/)return new_???();
...
Result</*???*/, /*???*/> deserializeOptionListField_JavaSerializer(/*???*/ fieldName, /*???*/ listType, /*???*/ node, Set</*???*/> consumedFields) {
	...
	{
	consumedFields.add(fieldName);
	Result<List</*???*/>, /*???*/> elementsResult=deserializeListElements(elementClass, value);
...
Result<List</*???*/>, /*???*/> deserializeListElements_JavaSerializer(Class</*???*/> elementClass, List</*???*/> nodeList) {
	/*???*/=new_???();
	/*???*/=new_???();
	/*???*/ index=/*???*/;
	/*???*/ i=/*???*/;
...
Result<Class</*???*/>, /*???*/> erase_JavaSerializer(/*???*/ type) {

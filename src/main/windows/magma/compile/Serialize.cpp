...
struct Serialize{};
template<typename T>
Result<T, CompileError> deserialize_Serialize(Class<T> clazz, Node node) {/*if (Objects.isNull(clazz))
			return new Err<>(new CompileError("Target class must not be absent", new NodeContext(node)));*//*if (Objects.isNull(node))
			return new Err<>(new CompileError("Cannot deserialize absent node", new StringContext(clazz.getName())));*//*return deserializeValue(clazz, node).mapValue(clazz::cast);*/}
template<typename T>
Result<Node, CompileError> serialize_Serialize(Class<T> clazz, T value) {/*if (Objects.isNull(clazz))
			return new Err<>(new CompileError("Target class must not be absent", new StringContext("serialize")));*//*if (Objects.isNull(value))
			return new Err<>(new CompileError("Cannot serialize absent instance of '" + clazz.getName() + "'",
					new StringContext("serialize")));*//*return serializeValue(clazz, value);*/}
Result<Node, CompileError> serializeValue_Serialize(Class</*?*/> type, Object value) {/*if (type.isSealed() && !type.isRecord()) {
...
Result<Node, CompileError> serializeSealed_Serialize(Class</*?*/> type, Object value) {/*final Class<?> concreteClass = value.getClass();*//*if (!type.isAssignableFrom(concreteClass)) {
...
Result<Node, CompileError> serializeRecord_Serialize(Class</*?*/> type, Object value) {/*Node result = createNodeWithType(type);*//*List<CompileError> errors = new ArrayList<>();*//*for (RecordComponent component : type.getRecordComponents()) {
			try {
...
Result<Node, CompileError> serializeField_Serialize(RecordComponent component, Object value) {/*String fieldName = component.getName();*//*Class<?> fieldType = component.getType();*//*if (Objects.isNull(value)) {
...
Result<Node, CompileError> serializeOptionField_Serialize(RecordComponent component, Object value) {/*String fieldName = component.getName();*//*if (!(value instanceof Option<?> option)) {
...
Result<Node, CompileError> serializeOptionListField_Serialize(char* fieldName, Type listType, Object content) {/*if (!(content instanceof List<?> list)) {
			return new Err<>(new CompileError("Optional List component '" + fieldName + "' is not a List instance",
					new StringContext(fieldName)));
		}*//*Type elementType = getGenericArgument(listType, 0);*//*Class<?> elementClass = erase(elementType);*//*return serializeListElements(elementClass, list)
				.mapValue(nodes -> nodes.isEmpty() ? new Node() : new Node().withNodeList(fieldName, nodes));*/}
Result<Node, CompileError> serializeListField_Serialize(RecordComponent component, Object value) {/*String fieldName = component.getName();*//*if (!(value instanceof List<?> list)) {
			return new Err<>(new CompileError("Component '" + fieldName + "' is not a List instance",
					new StringContext(fieldName)));
		}*//*Type elementType = getGenericArgument(component.getGenericType(), 0);*//*Class<?> elementClass = erase(elementType);*//*return serializeListElements(elementClass, list)
				.mapValue(nodes -> nodes.isEmpty() ? new Node() : new Node().withNodeList(fieldName, nodes));*/}
Result<List<Node>, CompileError> serializeListElements_Serialize(Class</*?*/> elementClass, List</*?*/> list) {/*List<Node> nodes = new ArrayList<>();*//*List<CompileError> errors = new ArrayList<>();*//*for (Object element : list) {
...
Result<Object, CompileError> deserializeValue_Serialize(Class</*?*/> type, Node node) {/*if (type.isSealed() && !type.isRecord()) {
...
Result<Object, CompileError> deserializeSealed_Serialize(Class</*?*/> type, Node node) {/*if (!(node.maybeType instanceof Some<String>(String nodeType))) {
			return new Err<>(new CompileError("Missing node type for sealed type '" + type.getName() + "'",
					new NodeContext(node)));
		}*//*// Try direct permitted subclasses*//*for (Class<?> permitted : type.getPermittedSubclasses()) {
		...
		}*//*// Try nested sealed interfaces*//*for (Class<?> permitted : type.getPermittedSubclasses()) {
			if (permitted.isSealed() && !permitted.isRecord()) {
				...
				new CompileError("No permitted subtype of '" + type.getName() + "' matched node type '" + nodeType + "'",
						new NodeContext(node)));*/}
Result<Object, CompileError> deserializeRecord_Serialize(Class</*?*/> type, Node node) {/*// Validate type annotation if present*//*Option<String> expectedType = resolveTypeIdentifier(type);*//*if (expectedType instanceof Some<String>(String expectedType0)) {
		...
		}*//*RecordComponent[] components = type.getRecordComponents();*//*Object[] arguments = new Object[components.length];*//*List<CompileError> errors = new ArrayList<>();*//*Set<String> consumedFields = new HashSet<>();*//*for (int i = 0;*//*i < components.length;*//*i++) {
		...
		}*//*// Validate that all fields were consumed*//*Option<CompileError> validationError = validateAllFieldsConsumed(node, consumedFields, type);*//*if (validationError instanceof Some<CompileError>(CompileError error)) {
			errors.add(error);
		}*//*if (!errors.isEmpty()) {
...
Result<Object, CompileError> deserializeField_Serialize(RecordComponent component, Node node, Set<String> consumedFields) {/*String fieldName = component.getName();*//*Class<?> fieldType = component.getType();*//*if (fieldType == String.class) {
		...
		}*//*Option<Node> childNode = node.findNode(fieldName);*//*if (childNode instanceof Some<Node> some) {
...
Result<Object, CompileError> deserializeStringField_Serialize(char* fieldName, Node node, Set<String> consumedFields) {/*Option<String> direct = node.findString(fieldName);*//*if (direct instanceof Some<String>(String value)) {
			consumedFields.add(fieldName);
			return new Ok<>(value);
		}*//*// Try nested search*//*Option<String> nested = findStringInChildren(node, fieldName);*//*if (nested instanceof Some<String>(String value)) {
			consumedFields.add(fieldName);
			return new Ok<>(value);
		}*//*else {
			return new Err<>(new CompileError("Required component '" + fieldName +
...
Result<Object, CompileError> deserializeOptionField_Serialize(RecordComponent component, Node node, Set<String> consumedFields) {/*Type elementType = getGenericArgument(component.getGenericType(), 0);*//*Class<?> elementClass = erase(elementType);*//*String fieldName = component.getName();*//*if (elementClass == String.class) {
...
Result<Object, CompileError> deserializeOptionListField_Serialize(char* fieldName, Type listType, Node node, Set<String> consumedFields) {/*Type elementType = getGenericArgument(listType, 0);*//*Class<?> elementClass = erase(elementType);*//*Option<List<Node>> maybeList = node.findNodeList(fieldName);*//*if (maybeList instanceof Some<List<Node>> some) {
			consumedFields.add(fieldName);
...
Result<Object, CompileError> deserializeListField_Serialize(RecordComponent component, Node node, Set<String> consumedFields) {/*String fieldName = component.getName();*//*Type elementType = getGenericArgument(component.getGenericType(), 0);*//*Class<?> elementClass = erase(elementType);*//*Option<List<Node>> maybeList = node.findNodeList(fieldName);*//*if (maybeList instanceof Some<List<Node>> some) {
			consumedFields.add(fieldName);
			Result<List<Object>, CompileError> elementsResult = deserializeListElements(elementClass, some.value());
...
Result<List<Object>, CompileError> deserializeListElements_Serialize(Class</*?*/> elementClass, List<Node> nodeList) {/*List<Object> results = new ArrayList<>();*//*List<CompileError> errors = new ArrayList<>();*//*for (Node childNode : nodeList) {
...
Node createNodeWithType_Serialize(Class</*?*/> type) {/*Node node = new Node();*//*Option<String> typeId = resolveTypeIdentifier(type);*//*if (typeId instanceof Some<String>(String value)) {
			node.retype(value);
		}*//*return node;*/}
Node mergeNodes_Serialize(Node base, Node addition) {/*Node result = new Node();*//*result.maybeType = base.maybeType;*//*// Merge nodes and node lists by creating new node and copying fields*//*// Note: We can't access private fields directly, so we create a new merged node*//*// by using the public merge method*//*result.merge(base);*//*result.merge(addition);*//*return result;*/}
Type getGenericArgument_Serialize(Type type, int index) {/*if (type instanceof ParameterizedType parameterized) {
			Type[] args = parameterized.getActualTypeArguments();
			if (args.length > index) {
				return args[index];
			}
		}*//*throw new IllegalArgumentException("Type " + type + " does not have generic argument at index " + index);*/}
Class</*?*/> erase_Serialize(Type type) {/*if (type instanceof Class<?> clazz) {
			return clazz;
		}*//*if (type instanceof ParameterizedType parameterized && parameterized.getRawType() instanceof Class<?> raw) {
			return raw;
		}*//*throw new IllegalArgumentException("Cannot erase type '" + type + "'");*/}
Option<String> resolveTypeIdentifier_Serialize(Class</*?*/> clazz) {/*Tag annotation = clazz.getAnnotation(Tag.class);*//*return Objects.isNull(annotation) ? Option.empty() : Option.of(annotation.value());*/}
Option<String> findStringInChildren_Serialize(Node node, char* key) {/*for (Node child : node.nodes.values()) {
...
boolean shouldBeDeserializableAs_Serialize(Node node, Class</*?*/> targetClass) {/*if (node.maybeType instanceof None<String>) {
			return false;
		}*//*if (node.maybeType instanceof Some<String>(String nodeType)) {
...
Option<CompileError> validateAllFieldsConsumed_Serialize(Node node, Set<String> consumedFields, Class</*?*/> targetClass) {/*// Collect all field names from the Node*//*Set<String> allFields = new HashSet<>();*//*allFields.addAll(getStringKeys(node));*//*allFields.addAll(node.nodes.keySet());*//*allFields.addAll(node.nodeLists.keySet());*//*// Find fields that were not consumed*//*Set<String> leftoverFields = new HashSet<>(allFields);*//*leftoverFields.removeAll(consumedFields);*//*if (!leftoverFields.isEmpty()) {
...
Set<String> getStringKeys_Serialize(Node node) {/*return node.getStringKeys();*/}

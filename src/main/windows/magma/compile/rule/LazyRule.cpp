...
void set_LazyRule(Rule rule) {/*maybeChild = new Some<>(rule);*/}
Result<Node, CompileError> lex_LazyRule(char* content) {/*return switch (maybeChild.map(child -> child.lex(content))) {
...
Result<String, CompileError> generate_LazyRule(Node node) {/*return switch (maybeChild.map(child -> child.generate(node))) {

...
Rule Statements_NodeListRule(char* key, Rule rule) {/*return new NodeListRule(key, rule, new FoldingDivider(new EscapingFolder(new StatementFolder())));*/}
Rule Delimited_NodeListRule(char* key, Rule rule, char* delimiter) {/*return new NodeListRule(key, rule, new DelimitedRule(delimiter));*/}
Rule Values_NodeListRule(char* key, Rule rule) {/*return new NodeListRule(key, rule, new FoldingDivider(new ValueFolder()));*/}
Result<Node, CompileError> lex_NodeListRule(char* input) {/*final ArrayList<Node> children = new ArrayList<>();*//*for (String segment : divider.divide(input).toList()) {
...
Result<String, CompileError> generate_NodeListRule(Node value) {/*Option<Result<String, CompileError>> resultOption = value.findNodeList(key).map(list -> {

...
Rule Node_NodeRule(char* key, Rule rule) {/*return new NodeRule(key, rule);*/}
Result<Node, CompileError> lex_NodeRule(char* content) {/*return rule.lex(content).mapValue(node -> new Node().withNode(key, node));*/}
Result<String, CompileError> generate_NodeRule(Node node) {/*return switch (node.findNode(key)) {

...
Rule Or_OrRule(/*Rule...*/ rules) {/*return new OrRule(Arrays.asList(rules));*/}
Result<Node, CompileError> lex_OrRule(char* content) {/*final ArrayList<CompileError> errors = new ArrayList<>();*//*for (Rule rule : rules)
...
Result<String, CompileError> generate_OrRule(Node node) {/*final ArrayList<CompileError> errors = new ArrayList<>();*//*for (Rule rule : rules) {

...
Rule First_SplitRule(Rule left, char* infix, Rule right) {/*final Splitter splitter = new InfixSplitter(infix, new FirstLocator());*//*return new SplitRule(left, right, splitter);*/}
Rule Last_SplitRule(Rule leftRule, char* infix, Rule rightRule) {/*final Splitter splitter = new InfixSplitter(infix, new LastLocator());*//*return new SplitRule(leftRule, rightRule, splitter);*/}
Rule Split_SplitRule(Rule left, Splitter splitter, Rule right) {/*return new SplitRule(left, right, splitter);*/}
Result<Node, CompileError> lex_SplitRule(char* input) {/*return switch (splitter.split(input)) {
...
Result<String, CompileError> generate_SplitRule(Node node) {/*return leftRule.generate(node)

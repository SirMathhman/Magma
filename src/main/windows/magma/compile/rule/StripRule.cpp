...
Rule Strip_StripRule(Rule rule) {/*return new StripRule("?", rule, "?");*/}
Rule Strip_StripRule(char* left, Rule rule, char* right) {/*return new StripRule(left, rule, right);*/}
Result<Node, CompileError> lex_StripRule(char* content) {/*return rule.lex(content.strip());*/}
Result<String, CompileError> generate_StripRule(Node node) {/*return rule.generate(node).mapValue(generated -> {

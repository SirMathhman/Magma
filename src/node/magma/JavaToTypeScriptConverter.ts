...
record JavaToTypeScriptConverter(Path sourceDir, Path targetDir) {
			...
			content = content.lines()
											 ...
											 .map(line -> {
												 ...
												 if (line.strip().startsWith("import ") && !line.strip().startsWith("import java.")) {
													 String importPath = line.strip().substring("import ".length(), line.strip().length() - 1);
													 ...
													 return "import { " + className + " } from './" + relativePath.substring(relativePath.lastIndexOf('/') + 1) + "/" + className + "';";
												 }
												 return line;
											 })
	...
	private static boolean isConstructorDeclaration((input: string) {
		...
		if (!methodName.equals("Main")) return false;

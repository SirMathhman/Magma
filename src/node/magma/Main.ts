...
export class Main {
    ...
    public static void main(args: String[]) {
        ...
        Path projectRoot = currentDir.endsWith("java") ? currentDir.getParent().getParent() : currentDir;

        Path sourceDir = projectRoot.resolve(Paths.get("src", "java"));
        Path targetDir = projectRoot.resolve(Paths.get("src", "node"));

        // Create a JavaToTypeScriptConverter to handle the conversion process
        JavaToTypeScriptConverter converter = new JavaToTypeScriptConverter(sourceDir, targetDir);

        System.out.println("=== Processing Java files from " + sourceDir + " to " + targetDir + " ===");
        System.out.println();

        Result<Integer, IOException> result = converter.processDirectory();

        // Use pattern matching with instanceof for the Result type
        switch (result) {
            case Err<Integer, IOException> err -> {
                Exception e = err.error();
                System.err.println("Error processing files: " + e.getMessage());
                //noinspection CallToPrintStackTrace
                e.printStackTrace();
            }
            case Ok<Integer, IOException> ok -> {
                System.out.println();

...
public class DiagnoseLangFile {
	...
	@Test
	public void diagnoseLangFile() throws Exception {
		...
		if (lexResult instanceof Ok<Node, CompileError>(Node lexedNode)) {
			System.out.println("\n✅ Lexing succeeded");
			...
			if (lexedNode.findNodeList("children") instanceof Some<java.util.List<Node>>(java.util.List<Node> children)) {
				...
				for (int i = 0; i < children.size(); i++) {
					...
					if (child.maybeType instanceof Some<String>(String type)) {
						...
						System.out.println("  Type char array: " + java.util.Arrays.toString(type.toCharArray()));

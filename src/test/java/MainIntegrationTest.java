...
public class MainIntegrationTest {
	...
	@Test
	public void testMainRunWritesFiles() {
		try {
			...
			Path windowsOut = temp.resolve("src").resolve("main").resolve("windows");
			Files.createDirectories(srcMain);
			Files.createDirectories(windowsOut);

			Path pkg = srcMain.resolve("magma");
			...
			Path sampleFile = pkg.resolve("Simple.java");
			...
			// compileAllJavaFiles
			var compileMethod = Main.class.getDeclaredMethod("compileAllJavaFiles", java.nio.file.Path.class,
					java.nio.file.Path.class);
			compileMethod.setAccessible(true);

			Object result = compileMethod.invoke(null, srcMain, windowsOut);
			assertTrue(result instanceof Option<?>);

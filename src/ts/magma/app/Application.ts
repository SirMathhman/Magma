...
class Application {
	...
	runWith(platform: Platform): Option<IOError> {
		return this.sources.listSources().flatMapNode((children: Iterable<Source>) => this.runWithChildren(platform, children)/*unknown*/).findError()/*unknown*/;
	}
	...
	runWithSource(state: CompileState, source: Source): Result<CompileState, IOError> {
		return source.read().flatMapNode((input: string) => this.runWithInput(state, source, input)/*unknown*/)/*unknown*/;
	}

...
export class StatefulOrRule<T> implements StatefulRule<T> {
	rules: Iterable<StatefulRuleAlias<T>>;
	constructor (rules: Iterable<StatefulRuleAlias<T>>) {
		this.rules = rules;
	}
	static compileOrPlaceholder(state: CompileState, input: string, rules: Iterable<StatefulRuleAlias<string>>): Tuple2<CompileState, string> {
		return new StatefulOrRule<string>(rules).apply(state, input).orElseGet(() => {
			return new Tuple2Impl<?>(state, Placeholders.generatePlaceholder(input))/*unknown*/;
		})/*unknown*/;
	}
	apply(state: CompileState, input: string): Option<Tuple2<CompileState, T>> {
		return this.rules.iter().map((statefulRule: StatefulRuleAlias<T>) => statefulRule.apply(state, input)/*unknown*/).flatMap(Iters.fromOption).next()/*unknown*/;
	}
}

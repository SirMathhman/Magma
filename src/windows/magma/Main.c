/*final */struct Main {
	...
	/*final*/ struct var withoutEnd = input.substring(0input.length() - 1);
	...
	/*final*/ struct var newParams = params.isEmpty(/*) ? "" : Main*/.compileValues(params/*Main::compileDefinition*/);
	...
	/*if*/(/*0*/ struct <= index) {
			/*final var beforeName = strip*/.substring(0index);
